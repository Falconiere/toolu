...
- **Workflow skills** — an 8-phase chain with a write step and a review step each: `brainstorm`, `spec`, `spec-review`, `plan`, `plan-review`, `execution`, `execution-review`, `test`. Plus three non-chain skills: `orchestrator` for delegating broad work across subagents, `debug` — a break-glass scientific-debug loop (reproduce → observe → hypothesize → isolate root cause → fix → regress) any phase can drop into, backed by language-agnostic evidence helpers (`debug-testfail`/`debug-stack`/`debug-log`) and an opt-in Sentry adapter, and `deep-research` — a brainstorm-first research pipeline: user-approved guiding questions fanned out across `research-agent` workers (exa-search + context7), verified, and delivered as a cited report under `docs/research/`.
