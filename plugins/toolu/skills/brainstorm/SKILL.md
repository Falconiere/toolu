...
3. **Ask in rounds, with options, until nothing material is open.** Put every
   *ask* dimension through the active host's structured user-input interface as
   defined in [the host mapping](../../workflows/host-mapping.md). Use up to four
   questions per call, 2–4 concrete options each, your recommendation first and
   labeled, and a preview whenever the choice is a shape the user should see.
   Batch a round; never trickle. Answers routinely open the next material fork;
   keep going until no answer would change the design. Never ask a question with
   one sane answer; state that default so the user can override it.
