#!/usr/bin/env bats
# session-start.sh publishes the context7 search wrapper at a stable path
...
@test "session-start: publishes wrapper symlink at \$CLAUDE_CONFIG_DIR/context7/search.sh" {
  run bash "$HOOK" <<<'{}'
  [ "$status" -eq 0 ]
...
@test "session-start: refreshes a stale symlink to the current target" {
  pub_root="$CLAUDE_CONFIG_DIR/context7"
  mkdir -p "$pub_root"
...
@test "session-start: idempotent (second run leaves the symlink identical)" {
  bash "$HOOK" <<<'{}'
  before=$(readlink "$CLAUDE_CONFIG_DIR/context7/search.sh")
  run bash "$HOOK" <<<'{}'
  [ "$status" -eq 0 ]
