...
- **`code-review:review` skill** — reviews the branch diff against what this repo's CI review bot (the `claude[bot]` verdict comment) flags, so the bot's verdict is clean on the first push instead of bouncing low/nit findings back as rework. It also records a clean `push-review` state, satisfying toolu's `push-review` gate.
