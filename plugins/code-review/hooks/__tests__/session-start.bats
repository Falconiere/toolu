...
setup() {
  ...
  SRC="$(cd "$(dirname "$BATS_TEST_FILENAME")/../../skills/review/scripts" && pwd)/write-state.sh"
}
...
@test "session-start: refreshes a stale symlink to the current target" {
  ...
  run bash "$HOOK" <<<'{}'
  [ "$status" -eq 0 ]
  [ "$(readlink "$pub_root/write-state.sh")" = "$SRC" ]
}
...
@test "session-start: NEVER clobbers a real file at the helper path" {
  ...
  before=$(cat "$pub_root/write-state.sh")
  run bash "$HOOK" <<<'{}'
  [ "$status" -eq 0 ]
  [ ! -L "$pub_root/write-state.sh" ]
  [ "$(cat "$pub_root/write-state.sh")" = "$before" ]
}
