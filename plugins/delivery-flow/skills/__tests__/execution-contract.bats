...
# Contract tests for the private execution reference and delivery readiness.

ROOT="$(cd "$(dirname "$BATS_TEST_FILENAME")/../../../.." && pwd)"
SKILL="$ROOT/plugins/delivery-flow/skills/delivery-flow/references/execution.md"
...
@test "execution owns the v2 review state and ready unified verdict" {
  grep -qiE 'toolu-review.*version: 2|version: 2.*toolu-review' "$SKILL"
  grep -Fq 'verdict.sh" status' "$SKILL"
  grep -qi 'overall: ready' "$SKILL"
}
...
@test "execution only delivers with authorization and all delivery prerequisites" {
  ...
  grep -qi 'GitHub API authentication' "$SKILL"
  grep -qi 'non-default branch' "$SKILL"
  grep -qi 'pr-babysit.*installed' "$SKILL"
}

@test "execution follows the required brainstorm phase" {
  grep -qiE 'brainstorm.*already run' "$SKILL"
}
